...
func detectBestFormat(r *http.Request, cfg *config.Config) string {
	accept := r.Header.Get("Accept")
	if cfg.AvifSupport && strings.Contains(accept, "image/avif") {
		return FormatAVIF
	}
...
func LocalRandomImageHandler(cfg *config.Config) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if len(params.Tags) > 0 || len(params.ExcludeTags) > 0 {
			allMetadata, metaErr := utils.MetadataManager.GetAllMetadata(context.Background())
			if metaErr != nil {
				logger.Warn("Failed to get metadata for random image filtering", zap.Error(metaErr))
			} else {
				for _, metadata := range allMetadata {
					if metadata.Orientation == orientation && matchesTags(metadata.Tags, params.Tags, params.ExcludeTags) {
		...
		// Fall back to directory scanning if metadata filtering is not requested or no results
		...
		bestFormat := detectBestFormat(r, cfg)
		if params.Format != "" {
